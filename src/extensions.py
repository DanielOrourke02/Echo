...
class Droped_Wallets(commands.Cog):
    def __init__(self, bot) -> None:
        self.bot = bot
        self.claimed_wallets = set()  # Set to keep track of claimed wallets


    async def get_drop_channel(self, server_id):
        with open("drop_channels.json", "r") as file:
            data = json.load(file)
            return data.get(str(server_id))


    async def set_drop_channel(self, server_id, channel_id):
        with open("drop_channels.json", "r") as file:
            data = json.load(file)
        data[str(server_id)] = channel_id

        with open("drop_channels.json", "w") as file:
            json.dump(data, file)
    ...
    async def drop(self, ctx):
        try:
            await self.set_drop_channel(ctx.guild.id, ctx.channel.id)
            
            embed = discord.Embed(
                title="Drop channel Set",
                description=f"This channel {ctx.channel.mention}, has been set as the wallet drop channel.",
                color=discord.Color.green()
            )
    ...
    async def wallet_drop_task(self):
        try:
            for guild in self.bot.guilds:
                channel_id = await self.get_drop_channel(guild.id)
                if channel_id:
                    channel = guild.get_channel(int(channel_id))
                    if channel:
                        amount = random.randint(20000, 40000)
                        if channel.id not in self.claimed_wallets:
                            embed = discord.Embed(title="Dropped wallet! 💰", description=f"A man just dropped his wallet. He didn't notice...", color=discord.Color.gold())

                            view = discord.ui.View()
                            button = discord.ui.Button(style=discord.ButtonStyle.green, label="Claim Wallet")

                            async def wallet_button_callback(interaction):
                                if interaction.user.id not in self.claimed_wallets:
                                    self.claimed_wallets.add(channel.id)
                                    self.claimed_wallets.add(interaction.user.id) 
                                    
                                    update_user_balance(interaction.user.id, amount)

                                    embed = discord.Embed(
                                        title="🎉 Wallet Claimed! 🎉",
                                        description=f"A lost wallet containing **{amount} zesty coins** was claimed by {interaction.user.mention}",
                                        color=discord.Color.green(),
                                    )
                                    embed.set_footer(text=f"Made by mal023")
                                    ...
                                    await interaction.response.send_message("You've already claimed this wallet!", ephemeral=True)  # Send ephemeral message

                            button.callback = wallet_button_callback
    ...
    @wallet_drop_task.before_loop
    async def before_wallet_drop_task(self):
...
async def Droped_Wallets_setup(bot):
    cog = Droped_Wallets(bot)
    cog.wallet_drop_task.start()  # Start the task when the cog initializes
